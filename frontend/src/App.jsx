...
export default function App() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        ...
        setGroqEnabled(Boolean(payload.groq_completion_enabled));
        setTwilioEnabled(Boolean(payload.twilio_sms_enabled));
      } catch {
        setBackend(false);
      ...
      sock.onmessage = (e) => {
        ...
        try {
          ...
          if (data.mode === 'gesture') {
            setGestureSmsStatus(data.sms_status || '');
          }

          if (canvasRef.current && webcamRef.current?.video) {
            drawBox(canvasRef.current, webcamRef.current.video, data.boxes, data.prediction, data.confidence);
          }
        } catch {}
      };
  ...
  const handleTranslate = useCallback(async () => {
    ...
    try {
      ...
      setTranslated(payload.translated_text || '');
    } catch (err) {
  ...
  useEffect(() => {
    ...
    setCompletions([]);
    setCompletionError('');
    if (mode !== 'alphabet' || !groqEnabled || text.length < 2) return undefined;
    ...
    const id = setTimeout(async () => {
      ...
      try {
        ...
        if (payload.enabled) {
          setCompletions(Array.isArray(payload.suggestions) ? payload.suggestions : []);
        }
      } catch (err) {
  ...
  }, [sentence, mode, groqEnabled]);

  const syncSentenceCommand = (nextSentence) => {
    if (wsRef.current?.readyState !== WebSocket.OPEN) return;
    wsRef.current.send(
      JSON.stringify({
        type: 'command',
        command: 'set_sentence',
        sentence: nextSentence,
      })
    );
  };

  const applyCompletion = (suggestion) => {
    const picked = (suggestion || '').trim();
    if (!picked) return;
    const parts = sentence.trim().split(/\s+/).filter(Boolean);
    const nextSentence = parts.length > 1
      ? `${parts.slice(0, -1).join(' ')} ${picked}`
      : picked;
    if (!nextSentence) return;
    setSentence(nextSentence);
    syncSentenceCommand(nextSentence);
  };
  ...
  const speakTranslated = () => {
    ...
    const utterance = new SpeechSynthesisUtterance(translated.trim());
    utterance.lang = langMap[transLang] || transLang;
    utterance.rate = 0.9;
  ...
  const clearSentence = () => {
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: 'command', command: 'clear' }));
    }
  };
  ...
  const selectedLang = TRANSLATE_LANGUAGES.find((lang) => lang.code === transLang);
        ...
        <div style={s.sidebar}>
            ...
            <div style={s.card}>
              ...
              <div style={s.grid}>
                {'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map((char) => (
                  <div key={char} style={s.lCell(letter?.toUpperCase() === char)}>{char}</div>
                ))}
              </div>
          ...
          {mode === 'phrase' && (
            <div style={s.card}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {['hello', 'thanks', 'iloveyou'].map((item) => (
                  <div key={item} style={s.pill(letter?.toLowerCase() === item, '#2dd4bf')}>{item}</div>
                ))}
              </div>
              <div style={s.confBar}><div style={s.confFill} /></div>
            </div>
          ...
          {mode === 'gesture' && (
            <div style={s.card}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {['Open', 'Close', 'Pointer', 'OK'].map((item) => (
                    <div key={item} style={s.pill(letter === item, '#f59e0b')}>{item}</div>
                  ))}
              </div>
              <div style={s.confBar}><div style={s.confFill} /></div>
              <div style={{ marginTop: '10px', fontSize: '12px', color: twilioEnabled ? '#cbd5e1' : 'rgba(255,255,255,.24)', lineHeight: '1.5' }}>
                {twilioEnabled
                  ? 'Open hand SMS alert is armed. Hold Open steadily to trigger.'
                  : 'Set Twilio env vars to enable SMS on Open hand detection.'}
              </div>
              {!!gestureSmsStatus && (
                <div style={{ marginTop: '8px', fontSize: '12px', color: gestureSmsStatus.toLowerCase().includes('failed') ? '#f87171' : '#fdba74' }}>
                  {gestureSmsStatus}
                </div>
              )}
            </div>
          ...
          {mode === 'alphabet' && (
            <div style={s.card}>
              <div style={s.cTitle}>Word Completion</div>
              <div style={s.completionBox}>
                {completionLoading ? (
                  <span style={{ color: 'rgba(255,255,255,.25)', fontSize: '13px' }}>Completing...</span>
                ) : completionError ? (
                  <span style={{ color: '#f87171', fontSize: '13px' }}>{completionError}</span>
                ) : completions.length ? (
                    ...
                    {completions.map((item, index) => (
                      <button
                        ...
                        style={{
                          ...
                          textAlign: 'left',
                          padding: '9px 12px',
                          borderColor: index === 0 ? 'rgba(251,146,60,.28)' : 'rgba(255,255,255,.14)',
                          color: index === 0 ? '#fdba74' : '#e2e8f0',
                          background: index === 0 ? 'rgba(251,146,60,.09)' : 'rgba(255,255,255,.04)',
                        }}
                  ...
                  <span style={{ color: 'rgba(255,255,255,.22)', fontSize: '13px' }}>
                    {sentence.trim().split(/\s+/).filter(Boolean).slice(-1)[0]?.length >= 2
                      ? 'No clean word suggestions yet for this partial word.'
                      : 'Spell a few letters to get three clean word suggestions.'}
                  </span>
                ) : (
                  <span style={{ color: 'rgba(255,255,255,.22)', fontSize: '13px' }}>Set GROQ_API_KEY to enable AI word completion.</span>
                )}
              </div>
            </div>
          )}
          ...
          <div style={{ ...s.card, fontSize: '12px', color: 'rgba(255,255,255,.28)', lineHeight: '1.7' }}>
            {mode === 'alphabet' && (
              <>
                <strong style={{ color: `${COL}bb` }}>Spelling:</strong> Show an ASL hand shape - hold it still until the orange bar fills - letter commits automatically. Backspace gesture removes last letter.
              </>
            )}
            {mode === 'phrase' && (
              <>
                <strong style={{ color: '#2dd4bf99' }}>LSTM:</strong> Needs 30 frames (~3s) to warm up. Recognises: hello - thanks - iloveyou
              </>
            )}
            {mode === 'gesture' && (
              <>
                <strong style={{ color: '#f59e0baa' }}>TFLite:</strong> Instant. Recognises: Open - Close - Pointer - OK hand shapes
              </>
            )}
          </div>
        </div>
